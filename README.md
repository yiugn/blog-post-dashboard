...
`.github/workflows/update-and-deploy.yml`이 매시 17분에 실행됩니다. 최초 전체 이력 수집 중 느리거나 실패한 Tilnote 페이지는 진행 상태를 `data/posts.json`에 체크포인트로 남기고 다음 실행에서 이어 받습니다. 전체 이력이 완성된 뒤에는 기존 글 ID를 기준으로 새 글만 증분 수집합니다. 데이터가 바뀌면 Actions 봇이 `data/posts.json`과 `data/posts.csv`를 커밋하고 GitHub Pages를 다시 배포합니다.
